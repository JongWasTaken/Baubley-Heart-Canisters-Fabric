...
public class ItemHeartAmulet extends TrinketItem implements ExtendedScreenHandlerFactory<ItemHeartAmulet.HeartAmuletData> {
    ...
    public record HeartAmuletData(ItemStack stack) {
        public static final PacketCodec<RegistryByteBuf, HeartAmuletData> PACKET_CODEC = PacketCodec.tuple(
                PacketCodecs.codec(ItemStack.CODEC),
                HeartAmuletData::stack,
                HeartAmuletData::new
        );
    }

    @Override
    public HeartAmuletData getScreenOpeningData(ServerPlayerEntity player) {
        return new HeartAmuletData(player.getMainHandStack());
    }
}
